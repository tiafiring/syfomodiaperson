...
const textSendtTilArbeidsgiverOgNav = (arbeidsgiver, orgnr, sendtTilNavDato) => {
...
const textSendtTilArbeidsgiver = (arbeidsgiver, orgnr, sendtTilArbeidsgiverDato) => {
...
const hentStatustekst = (soknad) => {
    ...
    const arbeidsgiver = soknad.arbeidsgiver && soknad.arbeidsgiver.navn
        ...
        : soknad.arbeidsgiver
            ? soknad.arbeidsgiver
            : null;
    const orgnr = soknad.arbeidsgiver && soknad.arbeidsgiver.orgnummer ? formaterOrgnr(soknad.arbeidsgiver.orgnummer) : null;
    const sendtTilArbeidsgiverDato = soknadSendtTilArbeidsgiver ? tilLesbarDatoMedArstall(soknad.sendtTilArbeidsgiverDato) : null;
    const sendtTilNavDato = soknadSendtTilNav ? tilLesbarDatoMedArstall(soknad.sendtTilNAVDato || soknad.innsendtDato) : null;
