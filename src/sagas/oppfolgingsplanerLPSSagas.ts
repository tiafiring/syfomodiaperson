...
import { get } from '../api';
import * as actions from '../actions/oppfolgingsplanerlps_actions';
import { PersonOppgave } from "../types/PersonOppgave";

export function* hentOppfolgingsplanerLPS(action: any, personOppgaveList: PersonOppgave[]) {
...
function* watchHentOppfolgingsplanerLPS() {
    yield takeEvery(actions.HENT_OPPFOLGINGSPLANER_LPS_FORESPURT, hentOppfolgingsplanerLPSHvisIkkeHentet);
}

export default function* oppfolgingsplanerLPSSagas() {
    yield all([
       fork(watchHentOppfolgingsplanerLPS),
    ]);
}
