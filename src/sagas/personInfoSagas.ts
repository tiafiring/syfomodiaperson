...
import { get } from '../api';
import * as actions from '../actions/personInfo_actions';
...
function* watchHentPersonAdresse() {
    yield takeEvery(actions.HENT_PERSON_ADRESSE_FORESPURT, hentPersonAdresseHvisIkkeHentet);
}

export default function* personAdresseSagas() {
    yield all([
       fork(watchHentPersonAdresse),
    ]);
}
