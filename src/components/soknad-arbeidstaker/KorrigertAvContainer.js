...
export const KorrigertAv = ({ korrigertAvSoknad, fnr }) => {
    return (<AlertStripe type="info" className="blokk">
        <p className="sist">
            {textKorrigert(tilLesbarDatoMedArstall(getTidligsteSendtDato(korrigertAvSoknad)))}
        </p>
        <p className="sist">
            <Link className="lenke" to={`/sykefravaer/${fnr}/sykepengesoknader/${korrigertAvSoknad.id}`}>{texts.lenketekst}</Link>
        </p>
    </AlertStripe>);
};
